...
export const polyType = "poly";
export const outputType = "exit";
...
export let register = (name, fn, schema) => {
  ...
  Node.prototype["_" + name] = function () {
    // bypass shortcut
    return this;
  };
  return fn;
...
export function evaluate(code, scope) {
  let nodes = [];
  const out = register("out", function (input, channels = [0, 1]) {
    nodes.push(input.output(channels));
    return input;
  });
  if (scope) {
    scope.out = out;
