  ...
  ins: [
    {
      ...
      default: 1,
      description:
        "if 0, the noise will hold the previous value. defaults to 1.",
    },
  ],
  compile: ({ lang, vars: [next = 1], ...meta }) =>
    langs[lang].defUgen(meta, next),
});

export let rng = registerNode("lcgnoise", {
  ugen: "LcgNoise",
  tags: ["source", "noise"],
  ...
  compile: ({ lang, vars: [next = 1, reset = 0], ...meta }) =>
    langs[lang].defUgen(meta, next, reset),
});
...
export let midivel = registerNode("midivel", {
  ugen: "MidiVel",
  tags: ["external", "midi"],
  description:
...
export let midicc = registerNode("midicc", {
  ...
  ins: [
    ...
    { name: "initValue", default: -1 }, // could not name it "default" because syntax error
  ],
...
export let qf = registerNode("qf", {
  ...
  ins: [
    { name: "input", default: 0 },
    {
      ...
      default: 0,
      description:
        "filter type: 0 = lowpass, 1 = highpass, 2 = band pass, 3 = notch, 4 = allpass, 5 = peaking, 6 = lowshelf, 7 = highshelf",
    },
...
export let qlpf = register(
  "qlpf",
  (input, freq, q = 10) => input.qf(0, freq, q),
  {
...
export let qhpf = register(
  "qhpf",
  (input, freq, q = 10) => input.qf(1, freq, q),
  {
...
export let qbpf = register(
  "qbpf",
  (input, freq, q = 10) => input.qf(2, freq, q),
  {
...
export let qnf = register(
  "qnf",
  (input, freq, q = 10) => input.qf(3, freq, q),
  {
...
export let scope = register(
  "scope",
  (input, id) => {
    let channels = 1;
    if (input.type === "poly") {
      channels = input.ins.length;
    }
    const node = getNode("scope", input, id, channels);
    if (node.type !== "poly") {
      // always channel 0 when not poly
      node.ins.push({ type: "n", value: 0, ins: [] });
      return node;
    }
    // push channel index as argument for each scope channel
    node.ins.forEach((n, i) => n.ins.push({ type: "n", value: i, ins: [] }));
    return node;
  },
  {
    ugen: "Scope",
    ...
    compile: ({ vars: [input, id, channels, channel], ...meta }) => {
      return js.defUgen(meta, input, id, channels, channel);
    },
  }
);
